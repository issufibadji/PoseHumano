...
void getValidPairs(const std::vector<cv::Mat>& netOutputParts,
	...
	for(int k = 0; k < mapIdx.size();++k ){
		...
		int nB = candB.size();
		/*
		# Se forem detectados pontos-chave para o par de juntas
		# verifica cada junta em candA com cada junta em candB
		# Calcular o vetor distância entre as duas juntas
		# Encontre os valores de PAF em um conjunto de pontos interpolados entre as juntas
		# Use a fórmula acima para calcular uma pontuação para marcar a conexão como válida
		*/
...
int main(int argc,char** argv) {
	std::string inputFile = "./images/img1.png";
	std::string device = "cpu";
	//std::cout << "USO : ./pose-com-varias-pessoas <inputFile> <device>" << std::endl;
	if (argc == 2){   
...
/** indicar caminho de 2 arq de modelos treinados no Caffe Deep Learning Framework */
//Esficação de arquitetura da rede neural e armazenamento dos pesos do modelo treinado
	cv::dnn::Net inputNet = cv::dnn::readNetFromCaffe("./coco/pose_deploy_linevec.prototxt","./coco/pose_iter_440000.caffemodel");
	
	
	if (device == "cpu"){
	std::cout << "Uso de  CPU device" << std::endl;
        inputNet.setPreferableBackend(cv::dnn::DNN_TARGET_CPU);
    }
    else if (device == "gpu"){
	std::cout << "Uso GPU device" << std::endl;
	...
	std::cout << "Tempo de passagem para frente = " << std::chrono::duration_cast<std::chrono::milliseconds>(finishTP - startTP).count() << " ms" << std::endl;
	...
	for(int i = 0; i < nPoints;++i){
		...
		std::cout << "Pontos-chave - " << keypointsMapping[i] << " : " << keyPoints << std::endl;
